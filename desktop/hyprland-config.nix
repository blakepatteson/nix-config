...
in
{
  ...
  system.activationScripts.hyprland-config = /* bash */ ''
    ...
    # Get all window addresses as array
    mapfile -t WINDOWS < <(hyprctl clients -j | jq -r '.[].address')
    ...
    EOF
        chmod +x /home/blake/.config/hypr/scripts/global-window-cycle.sh
        chown blake:users /home/blake/.config/hypr/scripts/global-window-cycle.sh
  '';
}
